<div class="py-12 {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            <!-- Title -->
            <h2 class="text-2xl font-semibold mb-6">{{ __('messages.item_card') }}</h2>
                ...
                <div class="overflow-x-auto bg-white rounded-lg shadow">
                    <table class="min-w-full divide-y divide-gray-200 border">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wider border {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('messages.date') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wider border {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('messages.document_no') }}</th>
                                <th class="px-6 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wider border {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('messages.department') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($movements as $index => $movement)
                                <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-gray-100">
                                    ...
                                    <td class="px-6 py-3 text-sm border text-center {{ $movement['in_quantity'] ? 'text-red-600 font-medium' : 'text-gray-900' }}">
                                        {{ $movement['in_quantity'] ?? '-' }}
                                    </td>
                                    <td class="px-6 py-3 text-sm text-gray-900 border text-center">{{ $movement['out_quantity'] ?? '-' }}</td>
                                    <td class="px-6 py-3 text-sm text-gray-900 border text-center">{{ $movement['balance'] }}</td>
                                </tr>
                            @endforeach
                            <!-- Totals Row -->
                            <tr class="bg-gray-100 font-semibold">
                                ...
                                <td class="px-6 py-3 text-sm text-red-600 border text-center">{{ $totalIn }}</td>
                                <td class="px-6 py-3 text-sm text-gray-700 border text-center">{{ $totalOut }}</td>
                                <td class="px-6 py-3 text-sm text-gray-700 border text-center">{{ $balance }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
